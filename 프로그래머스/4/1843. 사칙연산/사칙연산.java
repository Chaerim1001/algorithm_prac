public class Solution {
    ...
    private static int cal(int flag, int start, int end) {
        if (start == end) {
        ...
        if (flag == 0) {
            for (int mid = start; mid < end; mid++) {
                if (operations[mid].equals("-")) {
                    result = Math.max(result, cal(0, start, mid) - cal(1, mid + 1, end));
                    continue;
                }
                result = Math.max(result, cal(0, start, mid) + cal(0, mid + 1, end));
            }
        }
        ...
        if (flag == 1) {
            for (int mid = start; mid < end; mid++) {
                if (operations[mid].equals("-")) {
                    result = Math.min(result, cal(1, start, mid) - cal(0, mid + 1, end));
                    continue;
                }
                result = Math.min(result, cal(1, start, mid) + cal(1, mid + 1, end));
            }
        }
